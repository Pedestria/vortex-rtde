...
export function SearchAndGraph(entry:QueueEntry,Graph:VortexGraph,planetName?:string){
    ...
    traverse(entry.ast,{
        ImportDeclaration:function(path) {
            ...
            if(notNativeDependency(path.node.source.value)){
                Transport(resolveNonNativeDependency(LocalizedResolve(entry.name,path.node.source.value),impLoc),Graph,entry.name,null,planetName)
            } 
